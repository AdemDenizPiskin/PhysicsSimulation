...
spring1_pos = np.array([0,500])

#Fx,Fy = Gravity_source(G_pos,G,dr,grid_x,grid_y)

#Test_F = Force_Field(Fx, Fy, grid_x, grid_y)
test_particle = particle(np.array([500 * dr, 500 * dr]),np.array( [30, 0]), 1, 99.9, 99.9)
spring1 = spring(spring1_pos*dr,1.0,0.2,40)
...
for i in range(N):
    ...
    test_particle.update_F(spring1.calc_force(test_particle.pos,test_particle.vel))
...
def animate(i):
    ...
    line.set_ydata([50,y[i]] ) # update the data.
    return line,
